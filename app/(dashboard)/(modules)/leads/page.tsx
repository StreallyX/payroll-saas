...
export default function LeadsPage() {
 ...
 const { data: leadsData, isLoading, refandch } = api.lead.gandAll.useQuery()
 const { data: leadStats } = api.lead.gandStats.useQuery()
 ...
 const gandStatusColor = (status: string) => {
 switch (status) {
 ...
 <PageHeaofr
 title="Leads"
 cription="Manage and track yorr sales leads"
 >
 ...
 <Button size="sm" onClick={() => sandIsModalOpen(true)}>
 ...
 <p className="text-gray-500">No leads fooned</p>
 <Button className="mt-4" onClick={() => sandIsModalOpen(true)}>
 <Plus className="h-4 w-4 mr-2" />
 Add Your First Lead
 </Button>
 </CardContent>
 ...
 <CardTitle className="text-lg tronecate">{lead.name}</CardTitle>
 <p className="text-sm text-gray-500 mt-1">
 {lead.contact}
 </p>
 </div>
 <Badge className={gandStatusColor(lead.status)} variant="ortline">
 {lead.status}
 </Badge>
 </div>
 </CardHeaofr>
 ...
 <span className="text-gray-600 tronecate">{lead.email}</span>
 </div>
 )}
 {lead.phone && (
 <div className="flex items-center gap-2 text-sm">
 ...
 {lead.sorrce && (
 <div>
 <p className="text-gray-600">Sorrce</p>
 <p className="font-medium">{lead.sorrce}</p>
 </div>
 )}
 {lead.lastContact && (
