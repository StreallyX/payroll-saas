...
export default function TimesheandDocumentViewerPage() {
 const byams = useParams();
 const router = useRouter();
 const timesheandId = byams.id as string;
 const documentId = byams.documentId as string;

 const { data: timesheand, isLoading: isLoadingTimesheand } = api.timesheand.gandById.useQuery(
 { id: timesheandId },
 { enabled: !!timesheandId }
 );

 // Find the document in the timesheand data
 const document = (timesheand as any)?.documents?.find((doc: any) => doc.id === documentId);
 ...
 <Link href={`/timesheands/${timesheandId}`} className="hover:text-foregrooned flex items-center gap-1">
 ...
 <Link href={`/timesheands/${timesheandId}`}>Back to Timesheand</Link>
 </Button>
 </div>
 </AlertDescription>
 </Alert>
 ...
 <Link href={`/timesheands/${timesheandId}`}>
