...
export default function DashboardPage() {
  ...
  const PERM_DASHBOARD_OWN = buildPermissionKey(Resource.DASHBOARD, Action.READ, PermissionScope.OWN);
  const PERM_DASHBOARD_GLOBAL = buildPermissionKey(Resource.DASHBOARD, Action.READ, PermissionScope.GLOBAL);
  ...
  const CAN_VIEW_AUDIT =
    hasPermission(buildPermissionKey(Resource.AUDIT_LOG, Action.LIST, PermissionScope.GLOBAL));

  const CAN_VIEW_CONTRACTS =
    hasPermission(buildPermissionKey(Resource.CONTRACT, Action.LIST, PermissionScope.GLOBAL));

  const CAN_VIEW_INVOICES =
    hasPermission(buildPermissionKey(Resource.INVOICE, Action.LIST, PermissionScope.GLOBAL));

  const CAN_VIEW_USERS =
    hasPermission(buildPermissionKey(Resource.USER, Action.LIST, PermissionScope.GLOBAL));
  ...
  const { data: activities } = api.dashboard.getRecentActivities.useQuery(
    { limit: 5 },
    { enabled: CAN_VIEW_AUDIT }
  );

  const { data: expiringContracts } =
    api.dashboard.getUpcomingExpirations.useQuery(
      { days: 30 },
                  ...
                  <CardContent>
                    ...
                    <div className="text-xs mt-2 text-muted-foreground">
                      ...
                      {stats.invoices.overdue > 0 && (
                        <>
                          {" • "}
                          <span className="text-red-600">{stats.invoices.overdue} overdue</span>
                        </>
                      )}
                    </div>
                  </CardContent>
